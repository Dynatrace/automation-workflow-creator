...
- Add the following objective example by defining the name, a DQL, and specifying a target as well as a warning threshold.`,
    ...
    {
      'dt.markdown': `- **Objective name**: Error rate
...
| summarize errorRate = sum(errors)/count() * 100`,
    ...
    {
      'dt.markdown': `- **Set criteria for this objective**: \`Lower than the defined failure value is better for my objective\`
- **Target**: \`2\`
- **Warning**: \`1.5\`
...
- Click on **Validate** to perform a manual validation of the objective.`,
    ...
    {
      'dt.markdown': `# Automation workflow
...
- \`validation_status\`: Status out of this set: { pass | warning | fail | info | error } that expresses the overall validation result.`,
    },
    {
      'dt.markdown': '# Trigger validation and fetch result',
    },
    {
      'dt.markdown': '## Step 1: Trigger validation by sending an event to Dynatrace',
    },
    {
      'dt.markdown': `- Click **Run function** to execute the JavaScript code below.
...
export default async function () {
  ...
  return event['execution_context'];
    ...
    {
      'dt.markdown': `## Step 2: Copy context ID
- Copy the ID returned by the previous JavaScript function.`,
    },
    {
      'dt.markdown': '## Step 3: Fetch the validation result by providing the context ID',
    },
    {
      'dt.markdown': `- Paste the ID into the DQL.
